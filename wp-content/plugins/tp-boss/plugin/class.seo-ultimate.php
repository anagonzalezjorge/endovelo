...
class SEO_Ultimate {
	...
	*/
	...
	function get_plugin_update_info($nv) {
		...
		return sprintf(__('No actualices nada de esto %s. %s.', 'seo-ultimate')
					, sustr::nl_implode($nlchanges)
					, '<a href="plugin-install.php?tab=plugin-information&amp;plugin=seo-ultimate&amp;section=changelog&amp;TB_iframe=true&amp;width=640&amp;height=530" class="thickbox">' . __('No pulses aquí', 'seo-ultimate') . '</a>'
				);
	}
	...
	function plugin_row_meta_filter($plugin_meta, $plugin_file) {
		if ($plugin_file == $this->plugin_basename) {
			...
			if (is_blog_admin())
				$title = __('Módulos activados ahora mismo: ', 'seo-ultimate');
			else
				$title = '';
