...
class SU_FofsLog extends SU_Module {
	...
	static function get_module_title() { return __('Registro E404', 'seo-ultimate'); }
	function get_module_subtitle() { return __('Registro E404', 'seo-ultimate'); }
	...
	function get_admin_table_columns() {
		return array(
			...
			, 'url' => __('URL con el 404', 'seo-ultimate')
			, 'last-hit-time' => __('Fecha o Hit más reciente', 'seo-ultimate')
			, 'referers' => __('Referers', 'seo-ultimate')
			, 'user-agents' => __('Que BOT fué?', 'seo-ultimate')
		);
	}
		...
		if (count($the404s)) {
			...
			foreach ($the404s as $url => $data) {
				...
				if (count($data['referers'])) {
					...
					echo "<a href='#' class='su_toggle_up' data-toggle='su-404s-hit-$md5url-referers'>".__('Esconder la lista', 'seo-ultimate')."</a>";
					echo "</div>\n";
					echo "\t\t\t<ul>\n";
	...
	function clear_log_button() {
		...
		$confirm = __('Estás seguro que quieres eliminar todos los registros de errores 404?', 'seo-ultimate');
		echo "<div class='su-404s-clear-log'><a href=\"$clearurl\" class=\"button-secondary\" onclick=\"javascript:return confirm('$confirm')\">";
		_e('Limpiar el registro', 'seo-ultimate');
		echo "</a></div>";
	}
}
