...
class SU_ContentAutolinks extends SU_Module {
	...
	function get_module_subtitle() { return __('Tus enlaces', 'seo-ultimate'); }
	...
	function admin_page_contents() {
		...
		echo "\n<p>";
		_e('Haz un enlazamiento interno automático añadiendo una palabra o frase (texto de ancla), su destino y el atributo del título (cuando pasas el ratón por encima).', 'seo-ultimate');
		echo "</p>\n";
		...
		$this->admin_subheader(__('Añadir un nuevo enlace', 'seo-ultimate'));
	...
	function content_links_form($guid, $start_id = 0, $links, $delete_option = true) {
		...
		//Set headers
		$headers = array();
		$headers['link-anchor'] = __('Texto de ancla', 'seo-ultimate');
		$headers['link-to'] = __('Destino', 'seo-ultimate');
		$headers['link-title'] = __('Atributo <em>(ratón)</em>', 'seo-ultimate');
		...
		$headers['link-options'] = __('EXTRA', 'seo-ultimate');
		if ($delete_option)
			$headers['link-delete'] = __('Eliminar', 'seo-ultimate');
		...
		foreach ($links as $link) {
			...
			$cells['link-options'] .=
					 $this->get_input_element('checkbox', "link_{$i}_nofollow", $link['nofollow'], str_replace(' ', '&nbsp;', __('Dámelo NOFOLLOW', 'seo-ultimate')))
					.'<br />'
					.$this->get_input_element('checkbox', "link_{$i}_target", $link['target'] == 'blank', str_replace(' ', '&nbsp;', __('Abrir en nueva ventana', 'seo-ultimate')));
			if ($delete_option)
