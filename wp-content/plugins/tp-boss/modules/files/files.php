...
class SU_Files extends SU_Module {
	...
	static function get_module_title() { return __('Editor de .htaccess y robots.txt', 'seo-ultimate'); }
	static function get_menu_title() { return __('TPEditor HTyRob', 'seo-ultimate'); }
	...
	function admin_page_contents() {
		...
		$this->textarea('robotstxt', sprintf(__('robots.txt [<a href="%s" target="_blank">Abrir actual</a>]', 'seo-ultimate'), trailingslashit(get_bloginfo('url')).'robots.txt'));
		
		//Show robots.txt settings checkboxes
		$this->checkboxes(array(
			  'enable_custom_robotstxt' => __('Marca esta opción si quieres usar el robots.txt personalizado (el de arriba) y deshabilitar el que viene por defecto en tu sitio', 'seo-ultimate')
			, 'enable_do_robotstxt_action' => __('Marca esta opción si quieres que los plugins (el que sea) pueda editar "sin permiso" tu robots.txt', 'seo-ultimate')
		), __('Opciones del robots.txt', 'seo-ultimate'));
		
		$this->queue_message('warning',
			__('Recuerda que cualquier cambio realizado en estos dos archivos la puede liar parda... es recomendable que no le metas mucha mano si no sabes lo que haces', 'seo-ultimate'));
		...
		if ($is_super_admin && $is_apache && ($writable || !$exists)) {
			...
			$this->queue_message('warning',
				__('De todas formas siempre puedes hacer una copia de seguridad, por si las moscas!', 'seo-ultimate'));
		}
