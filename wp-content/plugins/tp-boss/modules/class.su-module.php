...
class SU_Module {
	...
	function meta_edit_tab($genus, $tab, $type, $type_label, $fields) {
		if (!$this->meta_edit_table($genus, $tab, $type, $type_label, $fields))
			$this->print_message('info', __('Qué pretendes cambiar? Actualmente no estás usando este contenido en tu sitio...', 'seo-ultimate'));
	}
	...
	function meta_edit_table($genus, $tab, $type, $type_label, $fields) {
		...
		$headers = array(
			  'actions' => __('Acciones', 'seo-ultimate')
		...
		foreach ($objects as $object) {
			...
			$actions = array(sprintf('<a href="%s">%s</a>', $view_url, __('Ver', 'seo-ultimate')));
			if ($edit_url)
				$actions[] = sprintf('<a href="%s">%s</a>', $edit_url, __('Editar', 'seo-ultimate'));
	...
	*/
	...
	function promo_sdf_banners() {
		...
		if ($this->should_show_sdf_theme_promo()) {	
			?>
			<img src="/wp-content/plugins/tp-boss/modules/sdf-ads/banners/seo-ultimate-plus-video-training.jpg" alt="TPBOSS" style="width:100%;height:100%;"></p>
			<img src="/wp-content/plugins/tp-boss/modules/sdf-ads/banners/seo-ultimate-plugin-done-for-you-service.jpg" alt="TPBOSS" style="width:100%;height:100%;"></p>
			<div id="sdf-promo-carousel"></div>
			<p><h4>Quieres acceder al mejor curso SEO?</h4><p>
			<h3><a href="http://vayaseo.com/teamplatino" target="_blank" title="TeamPlatino"><strong>Entrar a TeamPlatino ahora</strong></h3></p>
			<?php
		}	
	}
}
