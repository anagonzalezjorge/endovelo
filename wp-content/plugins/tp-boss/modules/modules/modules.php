...
class SU_Modules extends SU_Module {
	...
	static function get_module_title() { return __('Configuración de los Módulos', 'seo-ultimate'); }
	static function get_menu_title() { return __('Módulos', 'seo-ultimate'); }
	...
	function admin_page_contents() {
		...
		_e('Esto es una modificación totalmente gratuita del plugin SEO Ultimate. Al no haber actualización por parte del creador desde hace más de un año, decidí realizar una por mi cuenta.', 'seo-ultimate');
		echo "</p>\n<p>";
		_e('Si no eres suscriptor de <a href="http://www.vayaseo.com"><strong>vayaSEO.com</strong></a> ni estás dentro del curso de <a href="http://vayaseo.com/teamplatino"><strong>TeamPlatino</strong></a> no deberías tener este plugin... ¡¿quién te lo pasó?! xD', 'seo-ultimate');
		echo "</p></div>\n";
		
		if (!empty($_GET['su-modules-updated']))
			$this->print_message('success', __('Modules bien actualizados, eres dios!', 'seo-ultimate'));
		
		$this->admin_form_start(false, false);
		
		$headers = array(
			  __('Estado', 'seo-ultimate')
			, __('Módulo', 'seo-ultimate')
		);
...
STR;
		...
		$statuses = array(
			  SU_MODULE_ENABLED => __('Activado', 'seo-ultimate')
			, SU_MODULE_SILENCED => __('Modo silencio', 'seo-ultimate')
			, SU_MODULE_HIDDEN => __('Esconder', 'seo-ultimate')
			, SU_MODULE_DISABLED => __('Apagado', 'seo-ultimate')
		);
		$buttons = array(
